...
static int cmd_upload(int argc, char** argv) {
    ESP_LOGI(TAG, "Stopping player for update...");
    
    // 建議：先停止播放，確保 SD 卡和資源釋放，避免與 Update Task 搶資源
    Player::getInstance().stop(); 
    
    ESP_LOGI(TAG, "Starting TCP update task...");
...
static void register_all_commands(void) {
    ...
    register_cmd("upload", "download files from tcp server", &cmd_upload);
}
